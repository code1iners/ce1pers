...
/** Password generation error structure. */
interface PasswordGenerationError {
  /** Stable error code. */
  code: string;
...
export const usePassword = ({
  ...
  const charCode: number[] = Array.from(Array(26)).map((_, index) => index + 97);
  /** Lowercase letters available to the generator. */
  const lowercaseLetters: string[] = charCode.map((char) =>
    String.fromCharCode(char)
  );
  /** Uppercase letters available to the generator. */
  const uppercaseLetters: string[] = lowercaseLetters.map((char) =>
    char.toUpperCase()
  );
  ...
  const generate = () => {
    ...
    try {
      ...
      const cryptoError: PasswordGenerationError = {
        code: "0007",
